...
def main(markers, max_per_chrom_long=8, max_per_chrom_short=8, batches=8, distance=10e6):
    ...
    graph = LinkageGraph.populate(
        markers, max_per_chrom_long=max_per_chrom_long, max_per_chrom_short=max_per_chrom_short, batches=batches, distance=distance
    )
    panel = graph.panel
    for mh in panel:
        print(mh.chrom, mh.name, len(mh), mh.data.Ae)
...
class LinkageGraph(list):
    @classmethod
    def populate(cls, markers, batches=8, max_per_chrom_long=8, max_per_chrom_short=8, distance=10e6):
        graphs = cls()
        ...
        for chrom in chroms:
            ...
            for marker_set, max_per_chrom in zip((short, long), (max_per_chrom_short, max_per_chrom_long)):
                marker_set = sorted(marker_set, key=lambda mh: mh.data.Ae, reverse=True)
    ...
    def panel(self):
        ...
        maximal_cliques_per_batch = [list(nx.find_cliques(graph)) for graph in self]
...
if __name__ == "__main__":
    ...
    main(markers, max_per_chrom_short=args.max_per_chrom[0], max_per_chrom_long=args.max_per_chrom[1], batches=args.batches, distance=args.distance)
